				...
				try {
					...
					case 1: // add a new vehicle
						...
						Vehicle validVehicle = validateAllInputs(sc.next(), sc.next(), sc.nextDouble(), sc.next(),
								sc.next(), showroom);
						showroom.add(validVehicle);//instead of using array as we have used arraylist we can use add method to add data in array list
...
//								System.out.println("Removed " + showroom.remove(v));//Concurrent Modification Exception
//						
						// for loop
//						for (int i = 0; i < showroom.size(); i++)
//							if (showroom.get(i).getVehicleColor() == chosenClr)
//								System.out.println("Removed " + showroom.remove(i));//wrong output
					...
					case 10: // custom ordering using anonymous inner class
